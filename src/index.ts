...
export class Sniffer {
  ...
  stateBeforeTag() {
    const index = this.buffers[this.bufferIndex].indexOf(Chars.LT, this.index);
  ...
  process() {
    while (this.offset + this.index < SNIFF_BUFFER_SIZE) {
      ...
      } else if (this.state === State.MetaContentValueQuotedBeforeEncoding) {
        this.stateMetaContentValueQuotedBeforeEncoding(c);
      } else if (this.state === State.MetaContentValueQuotedAfterEncoding) {
      ...
      } else if (this.state === State.MetaContentValueQuotedValueUnquoted) {
        this.stateMetaContentValueQuotedValueUnquoted(c);
      } else if (this.state === State.MetaContentValueUnquotedBeforeEncoding) {
        this.stateMetaContentValueUnquotedBeforeEncoding(c);
      } else if (this.state === State.MetaContentValueUnquotedBeforeValue) {
        this.stateMetaContentValueUnquotedBeforeValue(c);
      } else if (this.state === State.MetaContentValueUnquotedValueQuoted) {
        this.stateMetaContentValueUnquotedValueQuoted(c);
      } else if (this.state === State.MetaContentValueUnquotedValueUnquoted) {
